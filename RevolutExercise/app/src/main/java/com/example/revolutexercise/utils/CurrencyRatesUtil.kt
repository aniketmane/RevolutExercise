        ...
        fun ratesResponseToRateList(context: Context, rates: Rates, baseCode: String, baseRate: Double): ArrayList<Rate> {
            ...
            rateList.add(
                Rate(
                    code = baseCode,
                    rate = baseRate,
                    flagImageResId = getFlagImageResId(context, baseCode)
                )
            )
                ...
                if (rate != null) {
                    rateList.add(
                        Rate(
                            code = member.name,
                            rate = rate * baseRate,
                            flagImageResId = getFlagImageResId(context, member.name)
                        )
                    )
                }
            }
            ...
            for (rate in rateListFromDB) {
                ...
                updatedRateList.add(
                    Rate(
                        ...
                        flagImageResId = getFlagImageResId(context, rate.code)
                    )
                )
            }
